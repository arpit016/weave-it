...
`weave-explore` and `weave-architect` use registered repos as bounded context sources. They lightly inventory all registered repos, then deeply inspect only relevant present repos based on the user request, active change artifacts, repo names/kinds, docs, knowledge, prior changes, or code references. They prefer current docs, knowledge specs, ADRs, and repo-local Weave wiki content before implementation code, and use code/tests to verify important claims.

Missing repos are unavailable local context, not an automatic hydration trigger. Skills mention a missing repo only when it affects the user's request, the active change, or confidence in the answer. If the missing repo may matter and a remote is recorded, the skill may tell the user that the repo needs to be cloned or added locally before that context can be inspected.
...
- Skills should use repo `availability` to distinguish registered-but-missing repos from present repos.
...
- Context-gathering skills should report which repos/docs/code anchors were inspected and which repos were skipped or missing when findings depend on workspace sub-repo context.
...
- Workspace display shape and repo availability: `src/lib/show-workspace.ts`
