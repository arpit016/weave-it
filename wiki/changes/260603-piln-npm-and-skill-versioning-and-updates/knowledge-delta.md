...
- **Plan Mode Protocol**: byte-identical `# Plan Mode Protocol` section embedded in `weave-explore`, `weave-prd`, `weave-architect`, `weave-clarify` (with `<lane>` placeholder substituted). Two-phase contract: in plan/ask/read-only mode, the agent declares the lane in plan output and ends with a "first action on acceptance" directive; in Agent Mode it makes `weave artifact current set <lane> --json` its first tool call.
- **Defensive Lane Verification in `weave-capture`**: `weave-capture` compares the resolved lane against the dominant subject of the conversation and explicitly asks the user when they disagree. Recovery path for failed Plan Mode Protocol Phase 2.
...
- New `change-workflow/domain-wide/` files: `lifecycle-progress-and-staleness.md`, `plan-mode-protocol.md`.
...
- `wiki/knowledge/domains/change-workflow/domain-wide/plan-mode-protocol.md` (created)
...
- `src/lib/skill-template-checks.ts` (`EXPECTED_NOTICE_BOILERPLATE`, `EXPECTED_PLAN_MODE_PROTOCOL`, `EXPECTED_LIFECYCLE_SYNC_PROTOCOL`)
- `scripts/bump-skill-versions.mjs`; `package.json` `release:bump-skills`
- Templates: every `templates/skills/<name>/SKILL.md` carries `last_changed_in: 0.1.0` and the notice block; design-discussion skills carry the plan-mode block; five progress-calling skills carry the lifecycle-sync block.
...
- Templates: `templates/skills/weave-capture/SKILL.md` (Defensive Lane Verification); `templates/skills/weave-{explore,prd,architect,clarify}/SKILL.md` (Plan Mode Protocol); `templates/skills/weave-{prd,architect,clarify,issues,capture}/SKILL.md` (Lifecycle Staleness Verification)
